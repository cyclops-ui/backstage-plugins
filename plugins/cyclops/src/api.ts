...
export interface ModuleApi {
    ...
    getTemplateInitialValues: (repo: string, path: string, version: string, sourceType: string) => Promise<any>;
    createModule: (moduleName: string, moduleNamespace: string, templateRef: any, values: any) => Promise<any>;
...
export class ModuleApiClient implements ModuleApi {
    ...
    async listModules(): Promise<any[]> {
        return await this.fetch<any[]>(`/modules/list`)
    }

    async listNamespaces(): Promise<string[]> {
        return await this.fetch<string[]>(`/namespaces`)
    }

    async getTemplateStore(): Promise<any[]> {
        return await this.fetch<any[]>(`/templates/store`)
    }
    ...
    async getTemplateInitialValues(repo: string, path: string, version: string, sourceType: string): Promise<any> {
        return await this.fetch<any>(`/templates/initial?repo=${repo}&path=${path}&commit=${version}&sourceType=${sourceType}`)
    }
    ...
    async createModule(moduleName: string, moduleNamespace: string, templateRef: any, values: any): Promise<any> {
        return await this.fetch<any>(`/modules/new`, ResponseType.Void, {
            method: "POST",
            body: JSON.stringify({
                name: moduleName,
